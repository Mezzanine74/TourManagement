...
@Component({
  selector: 'app-musteri-edit',
  templateUrl: './musteri-edit.component.html',
  styleUrls: ['./musteri-edit.component.scss']
})
export class MusteriEditComponent implements OnInit {
  ...
  apiUyrukUrl:any;  
  apiOtelUrl:any;  
  uyrukEditorOptions:any;  
  otelEditorOptions:any;  
  listPageUrl: any;
  id: string;
  store: any;
  ...
  constructor(private activatedRoute: ActivatedRoute, private httpClient:HttpClient, private router: Router) { 
    ...
    this.apiUrl = ():string => Constants.apiRoot+ Constants.apiMusteri;
    this.listPageUrl = ():string => Constants.musteriList;
    this.apiUyrukUrl = ():string => Constants.apiRoot+ Constants.apiUyruk;
    this.apiOtelUrl = ():string => Constants.apiRoot+ Constants.apiOtel;
    ...
    this.uyrukDataSource = new DataSource({
      store: new CustomStore({  
        key: "id",   
        loadMode: "raw",     
        load: ():any => {
          return this.httpClient.get(this.apiUyrukUrl()).toPromise().then((response: any) => {return response.data});
        }
      })
    });

    this.otelDataSource = new DataSource({
      store: new CustomStore({  
        key: "id",        
        loadMode: "raw",
        load: ():any => {
          return this.httpClient.get(this.apiOtelUrl()).toPromise().then((response: any) => {return response.data});
        }
      })
    });    
    ...
    this.uyrukEditorOptions = {
      dataSource: this.uyrukDataSource, 
      valueExpr: 'id', 
      displayExpr: 'uyrukAdi',
    };    

    this.otelEditorOptions = {
      dataSource: this.otelDataSource, 
      valueExpr: 'id', 
      displayExpr: 'otelAdi',
    };    
