...
@Component({
  selector: 'app-tur-bolge-servis-saatleri-edit',
  templateUrl: './tur-bolge-servis-saatleri-edit.component.html',
  styleUrls: ['./tur-bolge-servis-saatleri-edit.component.scss']
})
export class TurBolgeServisSaatleriEditComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute, private httpClient:HttpClient, private router: Router) { 
    ...
    this.apiUrl = ():string => Constants.apiRoot+ Constants.apiTurBolgeServisSaatleri;
    this.listPageUrl = ():string => Constants.turBolgeServisSaatleriList;
    this.apiBolgeUrl = ():string => Constants.apiRoot+ Constants.apiBolge;
    ...
    this.bolgeDataSource = new DataSource({
      store: new CustomStore({  
        key: "id",        
        loadMode: "raw",
        load: ():any => {
          return this.httpClient.get(this.apiBolgeUrl()).toPromise().then((response: any) => {return response.data});
        }
      })
    });
    ...
    this.editorOptions = {
      dataSource: this.bolgeDataSource, 
      valueExpr: 'id', 
      displayExpr: 'bolgeAdi',
    };    
